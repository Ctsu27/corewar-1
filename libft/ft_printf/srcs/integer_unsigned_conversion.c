...
/*   Updated: 2018/06/29 09:54:26 by kehuang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					t_ptf *env, char **bonus)
{
	...
	if (c == 'o' || c == 'O')
	{
		ptr = lo_itoa_base_unsigned(i, 8);
		if (env->dieze == 1 && ptr[0] != '0'
			&& env->precision <= lo_strlen(ptr))
			*bonus = lo_strdup("0");
	}
	else if (c == 'u' || c == 'U')
		ptr = lo_itoa_base_unsigned(i, 10);
	else if (c == 'x')
	{
		ptr = lo_itoa_base_unsigned(i, 16);
		if (env->dieze == 1 && ptr[0] != '0')
			*bonus = lo_strdup("0x");
	}
	else if (c == 'X')
	{
		ptr = lo_toupper_str(lo_itoa_base_unsigned(i, 16));
		if (env->dieze == 1 && ptr[0] != '0')
			*bonus = lo_strdup("0X");
	}
	return (ptr);
...
static	void		write_base(t_ptf *env, char *ptr, char *bonus)
{
	...
	printf_lo_putstr_buff(env, bonus);
	if (env->zero == 1)
		(env->neg == 0) ? printf_handle_width_other(env, big) : 0;
