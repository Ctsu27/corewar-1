...
/*   Updated: 2018/06/22 16:36:39 by clrichar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void		write_conv(t_ptf *env, long long x, int bigger, char *str)
{
	...
	size = lo_int_length(x);
	x < 0 ? printf_lo_putchar_buff(env, '-') : 0;
	...
	if (fake == 0)
	{
		env->frmt[env->index - 1] == 'b' ? str = lo_itoa_base(x, 2) : 0;
		env->frmt[env->index - 1] != 'b' ? str = lo_itoa_base(x, 10) : 0;
		if (!str)
			return ;
		else if (str[0] == '-')
			printf_lo_putstr_buff(env, str + 1);
		else
...
void				printf_d_conv(t_ptf *env, va_list *ap, int count)
{
	...
	x < 0 ? ++count : 0;
	bigger = count + (env->precision > lo_int_length(x) ? env->precision
			: lo_int_length(x));
